...
client = OpenAI(api_key="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")
...
def load_model(path="churn_pipeline.joblib"):
    """
    Load model from joblib. If the saved object is a dict, search
    for the first value with .predict and return it.
    """
...
# Try to load model (fail early but allow app to run)
try:
    model = load_model("churn_pipeline.joblib")
...
def select_feature_columns(df):
    """
    Prefer feature_0..feature_15. Otherwise return numeric columns
    excluding known result columns.
    """
...
def get_recommendation(prob):
    """Map churn probability to actionable business recommendation."""
    if prob is None:
...
def get_aggregate_recommendation(avg_prob):
    """Give company-wide action plan based on average churn probability."""
    if avg_prob is None:
...
# Track the latest predictions file during runtime
...
def upload_predict():
    """
    Upload a CSV, run batch predictions, save predictions file and show results.
    """
...
def predict_single():
    """
    Single-row prediction via form. Accepts feature_0..feature_15 or any numeric inputs.
...
def chatbot_api():
    ...
    full_prompt = (
        "You are an AI assistant for insurance churn analysis. Use these insights:\n"
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You analyze churn risk and provide insurance insights."},
